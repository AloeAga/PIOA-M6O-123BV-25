...
class JSONStorage:   
    def __init__(self, data_dir: str = "data") -> None:
        ...
        self._file_path = self.data_dir / f"{self._table_name}.json"
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise InvalidStorageDataError(f"Файл {self._table_name}.json повреждён")
        if "records" not in data:
            raise InvalidStorageDataError(f"Файл {self._table_name}.json имеет некорректную структуру")
        
        self._flats = [tuple(record) for record in data.get("records", [])]
    
    def _save(self) -> None:
        data = {"records": [list(record) for record in self._flats]}
