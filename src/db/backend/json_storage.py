...
class JSONStorage(Database):
    ...
    def __init__(self, data_dir: str = "data") -> None:
        ...
        self._load()
    def _load(self) -> None:
        """Загружает данные из JSON файла с проверкой структуры и типов."""
        ...
        try:
            ...
            raise InvalidStorageDataError(
                f"Файл {self.file_path.name} повреждён: некорректный JSON.\n"
                f"  Подробнее: {e}"
            )
        ...
        if "columns" not in data:
            raise InvalidStorageDataError(
                f"Файл {self.file_path.name} имеет некорректную структуру: отсутствует ключ 'columns'"
            )
        if "records" not in data:
            raise InvalidStorageDataError(
                f"Файл {self.file_path.name} имеет некорректную структуру: отсутствует ключ 'records'"
            )
        if data["columns"] != self.COLUMNS:
            raise InvalidStorageDataError(
                f"Файл {self.file_path.name} имеет некорректную структуру колонок.\n"
                f"  Ожидалось: {self.COLUMNS}\n"
                f"  Получено: {data['columns']}"
            )
        self._flats = []
        for row_num, record in enumerate(data.get("records", []), start=1):
            if not isinstance(record, list):
                raise InvalidStorageDataError(
                    f"Файл {self.file_path.name}, запись {row_num}: "
                    f"ожидается список, получено {type(record).__name__}"
                )
            if len(record) != 6:
                raise InvalidStorageDataError(
                    f"Файл {self.file_path.name}, запись {row_num}: "
                    f"ожидается 6 полей, получено {len(record)}"
                )
            try:
    ...
    def __repr__(self) -> str:
        return f"JSONStorage(file={self.file_path.name}, records={len(self._flats)})"