...
class JSONStorage(Database):
    """Хранилище в JSON файле. Данные сохраняются в data/flats.json."""
    
    # Определяем структуру таблицы (колонки)
    COLUMNS = ["id", "rooms", "street", "house", "square", "cost"]
    
    def __init__(self, data_dir: str = "data") -> None:
        self.data_dir = Path(data_dir)
        self.data_dir.mkdir(parents=True, exist_ok=True)
        self._file_path = self.data_dir / "flats.json"
        self._flats: list[FlatRecord] = []
        self._load()
    
    def _load(self) -> None:
        """Загружает данные из JSON файла (структуру таблицы и записи)."""
        ...
        try:
            ...
            raise InvalidStorageDataError("Файл flats.json повреждён")
        
        # Проверяем наличие структуры таблицы (как в методичке)
        if "columns" not in data:
            raise InvalidStorageDataError(
                f"Файл {self._file_path} имеет некорректную структуру: отсутствует ключ 'columns'"
            )
        
        if "records" not in data:
            raise InvalidStorageDataError(
                f"Файл {self._file_path} имеет некорректную структуру: отсутствует ключ 'records'"
            )
        
        # Проверяем соответствие колонок (опционально, но рекомендуется)
        if data["columns"] != self.COLUMNS:
            print(f"Внимание: структура таблицы в файле отличается от ожидаемой.")
            print(f"  Ожидалось: {self.COLUMNS}")
            print(f"  Получено: {data['columns']}")
        
        # Загружаем записи
        self._flats = [tuple(record) for record in data.get("records", [])]
    
    def _save(self) -> None:
        """Сохраняет данные в JSON файл (структуру таблицы и записи)."""
        data = {
            "columns": self.COLUMNS,  # Сохраняем структуру таблицы
